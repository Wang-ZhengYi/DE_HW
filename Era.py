...
def Era(O_m0=0.3,O_L0=0.7,O_r0=8.4e-5,O_k0=0,a_0=1,h=0.68,Ns=2000):
 ...
 #                   ))
	...
	text00 = r'$t_{m \Lambda} \approx 96.8 G yr$'
	ax1.text(1e-4,6,text0,fontsize=15)
	...
	ax1.text(1e-4,-5,textO,fontsize=14)
	# ax1.set_xlim(a.min(),a.max())
	# ax2.set_xlim(t.min(),t.max())
	ax2.set_xticklabels( ['{:5.2e}'.format(x) for x in np.logspace(t[1],t[-1],5,dtype='float')])
	# ax1.set_xticklabels(np.around(np.logspace(np.log10(a[1]),np.log10(a[-1]),7,dtype='float'),decimals=3))
	
	ax2.set_xlabel(r"$t/Gyr$",fontsize=15)
	ax1.minorticks_on()
	...
	fig1.tight_layout()
	fig1.savefig('R2M_Eralog.png',dpi=1200)
	plt.show()
	plt.clf()
